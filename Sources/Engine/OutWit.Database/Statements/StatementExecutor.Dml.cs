...
public sealed partial class StatementExecutor
{
                ...
                foreach (var whenClause in merge.WhenClauses)
                {
                    ...
                    switch (whenClause.ActionType)
                    {
                        ...
                        case MergeActionType.Delete:
                            ...
                            {
                                m_context.Database.DeleteRow(merge.TargetTable, matchedRowId.Value);
                                rowsAffected++;
                            }
                            break;
    ...
    /// </summary>
    private static WitSqlRow BuildMergeRow(WitSqlRow? targetRow, WitSqlRow sourceRow,
        string targetAlias, string sourceAlias)
    {
        var values = new List<WitSqlValue>();
        var names = new List<string>();
        ...
        if (targetRow != null)
        {
            for (int i = 0; i < targetRow.Value.ColumnCount; i++)
            {
                ...
                values.Add(targetRow.Value[i]);
                names.Add($"{targetAlias}.{colName}");

                // Also add without prefix for simple references
        ...
        string sourceAlias)
    {
        ...
        // Recalculate STORED computed columns
        var storedComputedColumns = targetTable.Columns.Where(c => c.IsComputed && c.IsStored).ToList();
        var intermediateRow = new WitSqlRow(newValues, columnNames);
        ...
        string sourceAlias)
    {
        ...
        var sourceValues = new List<WitSqlValue>();
        var sourceNames = new List<string>();
        for (int i = 0; i < sourceRow.ColumnCount; i++)
        {
            sourceValues.Add(sourceRow[i]);
            sourceNames.Add($"{sourceAlias}.{sourceRow.ColumnNames[i]}");
            // Also add without prefix
            sourceValues.Add(sourceRow[i]);
            sourceNames.Add(sourceRow.ColumnNames[i]);
        }
        var sourceRowForEval = new WitSqlRow([.. sourceValues], [.. sourceNames]);
